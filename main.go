...
func (dplr *deployer) postWebHook(w http.ResponseWriter, r *http.Request) {
	...
	if hookData.Repository.Namespace != `vitality710` {
...
func testAndDeploy() {
	testFiles, err := filepath.Glob("/app/deploymentTests/1-echotest.sh")
	if err != nil {
		panic(err)
	}
	var do_deploy = true
...
func deploy() {
	...
	params := &elasticbeanstalk.UpdateEnvironmentInput{
		ApplicationName: aws.String("Invoicer02"),
		EnvironmentId:   aws.String("e-mgbmrcccu6"),
		VersionLabel:    aws.String("Invoicer02-source"),
	}
...
"source": "https://github.com/hi-hee/deployer",
"version": "%s",
"commit": "%s",
"build": "https://circleci.com/gh/hi-hee/deployer/"
}`, version, commit)))
}
